...
# B_.full = pd.read_csv("B.csv").to_numpy().reshape((25,4,200))
B = np.eye(25)
...
Qm = 0.1*np.eye(Nx) #0*np.diag([4,4,4,.05,.05,.05])
Qcov = 0*Qm
...
constraints += [muN <= IN@(Acal@mu0 + Bcal@V)+1e-3*np.ones((Nx,1))]
constraints += [muN >= IN@(Acal@mu0 + Bcal@V)-1e-3*np.ones((Nx,1))]
...
rd_ellps = 5
h_ellps = 10
Elalp = np.zeros((Nx,1)) # np.array([0, 0, h_ellps - .1, 0, 0, 0]).reshape((Nx,1))

ElP = np.zeros((Nx,Nx)) #np.diag([(1/rd_ellps)**2,(1/rd_ellps)**2,(1/h_ellps)**2,0,0,0])
